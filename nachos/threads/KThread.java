...
public class KThread {
     ...
     * thread.
     */
    // 객체의 쓰레드가 끝날 때까지 기다린다. 만약 이 쓰레드가 이미 끝났다면 즉시 return한다. 이 메소드는 한 번만 호출되며 두 번째 호출부터는 return을 보장하지 않음
    // 객체의 쓰레드는 현재 쓰레드가 아니여야 함
    public void join() {
    ...
    private void Join() {
    	/*이 Joinqueue를 초기화 해주는 작업이 필요함
		 * 인터럽트를 막아주고 
		 * Join 메소드를 호출하면 매개변수로 전달받은 쓰레드에 해당 쓰레드를 달아놓는 역할을 수행(Joinqueue에 넣기)
		 * 부모 쓰레드가 수행을 마쳤을 때 기다리는 자식 쓰레드를 ready상태로 변경해서 실행하도록 해줘야함*/
    	/*
    	 * - 이미 A 쓰레드가 종료한 경우 B 쓰레드는 blocking되지 않고 바로 return함
		 * 그렇지 않은 경우, B 쓰레드는 A 쓰레드가 종료할 때까지 blocking되며(while문으로 기다림), A 쓰레드가 종료된 후(while 탈출 조건)
	  	 * join 메쏘드에서 return되어 나머지 작업을 수행함(쓰레드 반환?)*/
    ...
    private static ArrayList joined_thread = new ArrayList(); //자식 쓰레드 담아놓을 공간 생성
